...
const AlertAddElment: React.FC<MyComponentProps> = ({ name, myFunction, openAlert, setOpenAlert }) => {
    ...
    return (
      ...
      rounded-lg  w-full p-1.5 outline-none">
                        <input
                            ...
                            value={element.title}
                            onChange={(e) => {
                                setElement((pre: dataElmentDto) => { return { title: e.target.value, url: pre.url } });
                            }}
                        ></input>
                    </div>
      ...
      rounded-lg  w-full p-1.5 outline-none">
                        <input
                            ...
                            value={element.url}
                            onChange={(e) => {
                                setElement((pre: dataElmentDto) => { return { title: pre.title, url: e.target.value } });
                            }}
                        ></input>
                    </div>
                </DialogContent>
